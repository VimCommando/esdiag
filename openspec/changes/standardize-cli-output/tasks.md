...
- [ ] 1.4 Define internally tagged `CliOutcome` variants and compact allowlisted result structs for collect, process, upload, setup, host, keystore, job, and serve readiness operations.
...
- [ ] 2.4 Project every `JobRunOutcome` collect, upload, and process variant without losing archive paths, destinations, primary diagnostics, or included diagnostics.
- [ ] 2.5 Add exhaustive conversion tests for terminal workflow variants and absent optional links.
...
- [ ] 3.3 Replace saved-job list and delete rendering with typed outcomes while preserving all listing fields and empty collections.
...
- [ ] 4.5 Detect stdout-owned NDJSON workflows before execution and prevent YAML, JSON, prose summaries, or structured failures from being appended to a document stream.
...
- [ ] 6.1 Run formatting and targeted outcome, persistence, redaction, CLI, job, host, keystore, setup, and NDJSON stream tests.
- [ ] 6.2 Test all finite command families in both YAML and JSON, empty list outcomes, agent-mode parity, and structured non-zero failures.

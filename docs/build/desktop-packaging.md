...
- Builds use the committed notices by default so registry package verification and installation do not rewrite source files or require cargo-about. Set `ESDIAG_GENERATE_NOTICE=1` to regenerate NOTICE.txt and NOTICES.md during a source build, or explicitly run `cargo about generate --locked about.hbs -o NOTICE.txt` and copy its output to NOTICES.md during release preparation.

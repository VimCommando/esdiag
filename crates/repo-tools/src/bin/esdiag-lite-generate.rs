...
fn render_bash() -> Result<String> {
    ...
    writeln!(
        output,
        "# This region is generated by `cargo run -p repo-tools --bin esdiag-lite-generate`. Do not edit."
    )?;
    writeln!(output)?;
...
fn script_path(language: ScriptLanguage) -> PathBuf {
    ...
    repo_root().join("bin").join(filename)
}
...
fn main() -> Result<()> {
    ...
    for language in [ScriptLanguage::Bash, ScriptLanguage::PowerShell] {
        ...
        if check {
            if script != updated {
                bail!(
                    "{} is stale; run cargo run -p repo-tools --bin esdiag-lite-generate",
                    path.display()
                );
            }

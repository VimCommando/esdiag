...
#!/usr/bin/env bash
case "$1" in
  --version) printf 'esdiag %s\n' "${FAKE_ESDIAG_VERSION:-0.18.0-SNAPSHOT}" ;;
  setup) printf 'setup\n' >>"${FAKE_ESDIAG_LOG}" ;;
...
bash -n "$script"
[[ "$($script version)" == "0.18.0-SNAPSHOT" ]] || fail version
...
assert_contains "$tmp/mismatched-core.err" 'requires an ESDiag 0.18.0-SNAPSHOT binary'

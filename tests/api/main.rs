...
#[tokio::test]
async fn api_upload_non_zip_extension_returns_bad_request() {
    ...
    let url = format!("http://localhost:{}/api/upload", port);
...
#[tokio::test]
async fn api_upload_without_filename_returns_bad_request() {
    // Create a server instance
    let port = 9880;
    let _server = Server::new(
        port,
...
#[tokio::test]
async fn api_upload_invalid_zip_processes_and_returns_ready() {
    // Create a server instance
    let port = 9881;

...
printf '%s\n' '[package]' 'version = "0.18.0-SNAPSHOT"' >"$tmp/repo/Cargo.toml"
...
assert_contains "$tmp/runtime.log" 'build --file docker/Dockerfile . --tag esdiag:latest --tag esdiag:0.18.0-SNAPSHOT'
assert_contains "$tmp/delegate.log" 'image=esdiag:0.18.0-SNAPSHOT'
